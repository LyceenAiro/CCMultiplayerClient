...
export class OnPlayerChangeMapListener {
	...
	public async onPlayerChangeMap(player: string,
		...
		if (enters) {
			...
			// `skipHook` is our own marker (used to avoid re-broadcasting
			// network-spawned entities), not part of the game's typed Enemy
			// settings, so we build the settings object loosely.
